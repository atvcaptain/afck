...
if [ $# -lt 1 ]; then
	echo "Usage: $0 SPARSE_PRODUCT_IMAGE"
	exit
fi

ODM_IMAGE=$(readlink -f $1)

SIMG2IMG=simg2img
STOCK_FS=fs_config_orig.txt

script_root=$(dirname $(readlink -f $0))
tmp_name=`basename -s .sparseimg $PRODUCT_IMAGE`
tmp_mnt=$tmp_name
...
cd $script_root
mkdir -p $tmp_mnt
$SIMG2IMG $PRODUCT_IMAGE $tmp_img
